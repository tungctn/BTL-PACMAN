...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            # Kiểm tra xem hướng đi có bị tường chặn hay không
            x, y = state[0]
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]

            # Nếu không bị tường chặn, thêm vào danh sách các trạng thái kế tiếp
            if not hitsWall:
                next_pos = (nextx, nexty)
                visited_corners = set(state[1])

                if next_pos in self.corners and next_pos not in visited_corners:
                    visited_corners.add(next_pos)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Trạng thái hiện tại bao gồm vị trí và các góc đã đi qua
    ...
    # Nếu đã đi qua tất cả các góc, trả về 0
    ...
    # Tính toán khoảng cách Manhattan từ vị trí hiện tại đến góc gần nhất
    while unvisited_corners:
        closest_corner = min(
            unvisited_corners,
            key=lambda corner: util.manhattanDistance(current_pos, corner),
        )
        distance += util.manhattanDistance(current_pos, closest_corner)
...
class FoodSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        for direction in [
            ...
            # Thêm trạng thái kế tiếp vào danh sách các trạng thái kế tiếp
            x, y = state[0]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # # Tính toán maze distance từ vị trí hiện tại đến thức ăn xa nhất
    # farthest_food_dist = max(
    #     [mazeDistance(position, food, problem.startingGameState) for food in foodList]
    # )

    # return farthest_food_dist
    
    # Trạng thái hiện tại bao gồm vị trí và danh sách thức ăn
    position, foodGrid = state
    foodList = foodGrid.asList()
    ...
    # Tính toán khoảng cách Manhattan từ vị trí hiện tại đến thức ăn gần nhất
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        gameState.
        """
        # Sử dụng hàm AnyFoodSearchProblem để tìm đường đi đến thức ăn gần nhất
